...
空间复杂度为O(1),不表示只有1个或少数几个空间，而是说一个常量空间，可以是几千，几万。。
这里要求`待排序的数字在限定的范围内如【0，65535】`；这样我们可以使用hash sort,也就是分配一个hash[6535]的空间，初始化为0,遍历一遍，出现的数字就hash[k]++,这样再次遍历一次，就可以得到n个数的顺序了。
